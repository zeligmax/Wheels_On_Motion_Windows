...
#función que reciba la onda (numpy array) y el samplerate, y que dibuje los dos gráficos.
def mostrar_graficos(onda, samplerate=44100):
    ...
    ax.set_title("Espectro de frecuencias (FFT)", color='white')
    ax.set_xlabel("Frecuencia [Hz]", color='white')
    ax.set_ylabel("Magnitud", color='white')
    ax.tick_params(colors='white')
    ax.set_xlim(0, 2000)  # Limitar rango visible a 2000 Hz para mejor visualización
...
lbl_graficos = tk.Label(root, text="📈 Gráficos de sonido:", bg="#2b2b2b", fg="white", font=fuente)
lbl_graficos.pack(pady=(10, 0))

frame_graficos = tk.Frame(root, bg="#2b2b2b", height=300)
